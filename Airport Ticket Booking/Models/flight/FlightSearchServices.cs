...
public class FlightSearchServices : IFlightSearchServices
{
        ...
        decimal? maxPrice = null)
    {
        var searchResults = flights.Where(
            flight => CheckSearchCriteria(departureCountry, destinationCountry, departureDate, departureAirport, arrivalAirport, flightClass, maxPrice, flight)).ToList();
        return searchResults;
    }

    private static bool CheckSearchCriteria(string? departureCountry, string? destinationCountry, DateTime? departureDate, string? departureAirport, string? arrivalAirport, string? flightClass, decimal? maxPrice, Flight flight)
    {
